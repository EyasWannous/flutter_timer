...
void main() {
  ...
  group('TimerView', () {
    ...
    testWidgets(
      ...
      await tester.pumpTimerView(timerBloc);
      // await tester.tap(find.byIcon(Icons.play_arrow));
      // await tester.tap(find.byWidget(PlayButton(state: timerBloc.state)));
      // await tester.tap(find.byElementType(PlayButton));
      await tester.tap(find.byType(PlayButton));
      // verify(() => timerBloc.add(const TimerResumed())).called(1);
      verifyNever(() => timerBloc.add(const TimerResumed())).called(0);
    });
